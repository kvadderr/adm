...
const ClientAnal = () => {
  ...
  const dataFetch = async () => {
    setLoading(true);
    const data = await(
      await fetch("http://95.213.216.132:4000/analytics/client/"+startDay+"/"+endDay)
    ).json();
    setDataSource(data)
    setLoading(false);
  };
  ...
  for (let i = 0; i < dataSource.length; i++ ){
    data.push({
      ...
      talkcount: dataSource[i].talkcount,
      total: +dataSource[i].totalcost + +dataSource[i].totalcompany,
      lastpay: dataSource[i].lastpay,
      fav: dataSource[i].count,
    })
  }
  ...
  return (
    <>
      <Title>Аналитика по клиентам</Title>
